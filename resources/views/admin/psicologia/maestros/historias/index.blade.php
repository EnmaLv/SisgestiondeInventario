    ...
    <div class="pt-12 pb-4 bg-slate-50 dark:bg-gray-900 min-h-[calc(100vh-4rem)] flex flex-col">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex-1 flex flex-col w-full">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div>
                    <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Historial Clínico
                    </h1>
                    <p class="mt-2 text-slate-500 dark:text-gray-400 text-sm">Gestiona la evolución y expedientes de tus
                ...
                <div x-data="{ openModal: false }" class="w-full sm:w-auto">
                    <form action="{{ route('admin.psicologia.maestros.historias.index') }}" method="GET"
                        ...
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            <div x-data="{ open: false }" class="relative flex-1 sm:flex-initial">
                                <button @click="open = !open" @click.away="open = false" type="button"
                                    class="w-full sm:w-auto justify-center group flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-sm font-bold shadow-md shadow-indigo-200 dark:shadow-indigo-900/30 transition-all">
                                ...
                                <div x-show="open" x-transition x-cloak
                                    class="absolute left-0 sm:right-0 sm:left-auto mt-2 w-48 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-slate-100 dark:border-gray-700 overflow-hidden z-20">
                                    <a href="{{ route('admin.psicologia.maestros.historias.exportar.pdf', ['search' => request('search'), 'pnf' => request('pnf'), 'edad' => request('edad'), 'fecha_desde' => request('fecha_desde'), 'fecha_hasta' => request('fecha_hasta'), 'enfermedad_id' => request('enfermedad_id'), 'prioridad' => request('prioridad'), 'avance_id' => request('avance_id'), 'estado_animo_id' => request('estado_animo_id')]) }}"
                                        target="_blank"
                                        class="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors">
                                        <div
                                            class="w-8 h-8 rounded-lg bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-red-500">
                                    ...
                                    <a href="{{ route('admin.psicologia.maestros.historias.exportar.excel', ['search' => request('search'), 'pnf' => request('pnf'), 'edad' => request('edad'), 'fecha_desde' => request('fecha_desde'), 'fecha_hasta' => request('fecha_hasta'), 'enfermedad_id' => request('enfermedad_id'), 'prioridad' => request('prioridad'), 'avance_id' => request('avance_id'), 'estado_animo_id' => request('estado_animo_id')]) }}"
                                        class="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors">
                                        <div
                                            class="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <button type="button" @click="openModal = true"
                                class="flex-1 sm:flex-initial justify-center px-4 py-2.5 h-11 bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 font-bold text-sm rounded-2xl hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors flex items-center gap-2">
                        ...
                        <div class="relative w-full sm:w-64 lg:w-80">
                            ...
                            <svg class="w-5 h-5 absolute left-3 top-2.5 text-slate-400 dark:text-gray-500"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div x-show="openModal" x-cloak
                            ...
                            <div @click.away="openModal = false"
                                class="bg-white dark:bg-gray-800 rounded-3xl p-6 w-full max-w-md shadow-2xl border border-slate-100 dark:border-gray-700">
                                <div class="flex justify-between items-center mb-5">
                                    <h3 class="text-lg font-bold text-slate-800 dark:text-white">Filtros Avanzados</h3>
                                    <button type="button" @click="openModal = false"
                                        class="text-slate-400 hover:text-slate-600 dark:hover:text-gray-300">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <label
                                            class="block text-xs font-bold text-slate-500 dark:text-gray-400 mb-1">PNF/Carrera</label>
                                        <select name="pnf"
                                            ...
                                            @foreach ($pnfs as $key => $label)
                                                <option value="{{ $key }}"
                                                    {{ request('pnf') == $key ? 'selected' : '' }}>{{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                        <label
                                            class="block text-xs font-bold text-slate-500 dark:text-gray-400 mb-1">Edad</label>
                                    ...
                                    <div x-data="{ tipoFiltro: '{{ request('tipo_filtro_fecha', 'rango') }}' }">
                                        <label
                                            class="block text-xs font-bold text-slate-500 dark:text-gray-400 mb-1">Filtrar
                                            fechas por</label>
                                        <select name="tipo_filtro_fecha" x-model="tipoFiltro"
                                            class="w-full py-2.5 px-4 bg-slate-50 dark:bg-gray-700/50 border border-slate-200 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none text-gray-900 dark:text-white mb-3">
                                        ...
                                        <div class="grid grid-cols-2 gap-4" x-show="tipoFiltro !== ''" x-transition>
                                            <div>
                                                <label
                                                    class="block text-xs font-bold text-slate-500 dark:text-gray-400 mb-1">Fecha
                                                    desde</label>
                                        ...
                                        <p class="text-[11px] text-slate-400 dark:text-gray-500 mt-1.5 italic"
                                            x-show="tipoFiltro === 'primera_cita'">Mostrará pacientes cuya primera
                                            sesión realizada esté en este rango.</p>
                                        <p class="text-[11px] text-slate-400 dark:text-gray-500 mt-1.5 italic"
                                            x-show="tipoFiltro === 'ultima_cita'">Mostrará pacientes cuya última sesión
                                            realizada esté en este rango.</p>
                                        <p class="text-[11px] text-slate-400 dark:text-gray-500 mt-1.5 italic"
                                            x-show="tipoFiltro === 'rango'">Mostrará pacientes que tengan cualquier
                                            cita dentro de este rango.</p>
                                    </div>
                                ...
                                <div class="mt-6 flex justify-end gap-3">
                                    <a href="{{ route('admin.psicologia.maestros.historias.index') }}"
                                        class="px-4 py-2 bg-slate-100 text-slate-600 dark:bg-gray-700 dark:text-gray-300 font-bold text-sm rounded-xl hover:bg-slate-200 dark:hover:bg-gray-600 transition-colors">Limpiar</a>
                                    <button type="submit"
                                        class="px-5 py-2 bg-indigo-600 text-white font-bold text-sm rounded-xl hover:bg-indigo-700 shadow-md shadow-indigo-200 dark:shadow-indigo-900/20 transition-all">Aplicar
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-2 sm:px-4 lg:px-6">
                    @foreach ($historias as $historia)
                        ...
                        <div class="paciente-card bg-white dark:bg-gray-800 rounded-[32px] border border-slate-100 dark:border-gray-700 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-300 group overflow-hidden flex flex-col w-full"
                            ...
                            <!-- Footer Link -->
                            <a href="{{ route('admin.psicologia.maestros.historias.show', $paciente->id) }}"
